...
> **Scope:** standard mode — new tools, skill files and client methods are the web-API path. Agent-mode tools are not hand-written: their metadata is `docs/official-mcp-tools.json`, regenerated by `bun run refresh-official-tools` after `robinhood_official_login`. Which mode: [MODES.md](MODES.md).

## Adding a New MCP Tool (standard mode)
