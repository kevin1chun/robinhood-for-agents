...
> **Scope:** both modes. Rows tagged "(standard mode)" exist only in standard mode; every other tool exists in both (per-tool split: [official-mcp-tools.md](official-mcp-tools.md#parity)). In standard mode every call is relayed to Robinhood's hosted MCP and orders reach the Agentic account only. Which mode: [MODES.md](MODES.md).
...
- A parameter value the web API cannot serve (e.g. `dollar_amount` on equity orders, `tax_lots`, crypto stop orders) is rejected with a reason, never approximated
...
- No plaintext credential on disk: web-mode session tokens live in the OS keychain via `Bun.secrets` (default) or an AES-256-GCM file; the standard-mode credential lives only in an AES-256-GCM file keyed by `ROBINHOOD_TOKEN_KEY`, never the keychain
