...
Call `robinhood-for-agents:robinhood_check_session` (no parameters).
...
Tell the user that Chrome will open to the Robinhood login page, then call `robinhood-for-agents:robinhood_browser_login`.
...
### Step 3: Verify
After successful login, call `robinhood-for-agents:robinhood_get_account` to verify the session works. Confirm to the user that authentication is complete and all other skills will work automatically.
...
| Tool | Purpose |
|------|---------|
| `robinhood-for-agents:robinhood_check_session` | Check if already authenticated |
| `robinhood-for-agents:robinhood_browser_login` | Open Chrome for browser-based login |
| `robinhood-for-agents:robinhood_get_account` | Verify session works |
